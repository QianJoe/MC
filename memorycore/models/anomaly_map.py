...
from models.blur import GaussianBlur2d
...
class AnomalyMapGenerator(nn.Module):
    ...
    def compute_anomaly_map(self, patch_scores: torch.Tensor, feature_map_shape: torch.Size) -> torch.Tensor:
        ...
        anomaly_map = F.interpolate(anomaly_map, size=(self.input_size, self.input_size))
    ...
    def forward(self, **kwargs: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        max_anomaly_maps = [np.max(anomaly_map.detach().cpu().numpy())]
        anomaly_score = torch.tensor(max_anomaly_maps)
